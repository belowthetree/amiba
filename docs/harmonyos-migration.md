...
| SAF 文件夹选取 + 持久授权（fileAccess） | `DocumentViewPicker` + `fileshare` 持久化授权 | **无 MANAGE_EXTERNAL_STORAGE 对等物**：「按路径直扫共享目录」降级为「picker 授权目录」模型，fileAccess 模块功能收窄。**已实现**：壳层 `PickerCommands.ets`（file_access_pick_folder/list/read_text/read_binary 4 命令），前端 folder-picker/file-access-grants 加鸿蒙分支；picker URI（`file://docs/...`）经 `fs.openSync` 直读（d.ts 明确支持 URI），stat 走 fd（`fs.stat(uri)` 需 API 22）；`persistPermission` 需 ACL 受限权限 `ohos.permission.FILE_ACCESS_PERSIST`，best-effort 失败后降级为「仅本次生命周期有效」（与前端不落盘设计一致）。**服务文件夹导入**同样走此族（`ServiceBrowsePage.importFromPickerUri`：递归 list + 逐文件 read_text），**技能文件夹导入**亦走此族（`skills.ts importSkillFromFolder` + `storage.ts copySkillFolder` 鸿蒙分支：递归 list + 逐文件 read_binary，picker URI 判定与子项拼接统一收在 `folder-picker.ts` 的 `isHarmonyPickerUri`/`harmonyPickerChildUri`），picker URI 不可落 native-fs 沙箱通道。**真机踩坑（已解决，勿回退）**：① picker 必须经 `UIContext.runScopedTask` 切到 UI 线程拉起——javaScriptProxy 方法在 ArkWeb JS 桥线程执行，直调 `select()` 不弹窗也不返回（表现为前端「按钮没反应」；`onShowFileSelector` 路径在 UI 线程故 ZIP 导入正常），UIContext 由 EntryAbility 在 loadContent 后注入（此前 `getUIContext()` 抛 1300002）；② ArkWeb 不渲染未处理的 JS 对话框，壳层须接管 `onAlert`/`onConfirm`（AlertDialog 实现），否则前端 alert 错误提示与 fileAccess 的 `confirm()` 授权确认全部静默失效；③ 鸿蒙 picker 失败前端直接抛错（无可用回退：桌面 plugin-dialog 不存在，prompt 手输路径必被沙箱拒绝）；④ **`DocumentSelectMode.FOLDER` 依赖独立 syscap `SystemCapability.FileManagement.UserFileService.FolderSelection`（d.ts 标注，仅 PC/2in1/TV 等具备；手机无此 syscap，FOLDER 模式系统文件管理器弹「功能暂不支持」）**——壳层 `file_access_pick_folder` 经 `canIUse` 门控，无 syscap 自动降级 FILE 多选（≤500，前端经 `suffixes` 传后缀过滤），返回 `{files:[{uri,name,size}]}` 而非 `{uri}`：fileAccess 授权（`file-access-grants.ts`）据此构建文件清单授权（grant.files，listFiles 直接过滤、read 按文件名定位直读，同名取第一个为已知限制），服务零改动（音乐播放器手机端可用）；通用 `pickFolder`（服务/技能文件夹导入要的是目录结构，无法用文件清单替代）收到 files 响应抛「当前设备不支持选择文件夹」 |
