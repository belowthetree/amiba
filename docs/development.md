...
- **2026-07-17**: `index.json` 中的条目必须与实际目录严格一致。曾因 `index.json` 提交了 `user.remote_control` 条目但目录未提交，导致每次启动安装器都 fetch 失败并跳过（静默日志）。新增预置服务时必须同提交目录 + index.json 条目；校验方法：遍历 index.json 的 `files` 逐一 `Test-Path`。
- **2026-07-17**: Windows PowerShell 5.1 校验 JSON 时，`Get-Content` 对无 BOM 的 UTF-8 文件默认按 GBK 解码，中文内容会乱码导致 `ConvertFrom-Json` 误报语法错误。应使用 `[System.IO.File]::ReadAllText($path, [System.Text.UTF8Encoding]::new($false))` 显式按 UTF-8 读取后再解析。
...
- **2026-07-20**: 文件夹选取从手写 JNI (`FolderPickerHelper`) 迁移至 `tauri-plugin-android-fs` (v28.4.0)。…（详见上方完整条目）
- **2026-07-24**: `web_fetch` Rust HTTP 路径（`http_fetch`）新增 `raw` 字段返回原始 HTML。此前只返回 `extract_text()` 提取的纯文本，导致 `RemoteServicesPage.safeFetch` 下载服务源文件时 HTML/JS 内容被剥离为纯文本。修复：`FetchResult` struct 加 `raw: String`，`http_fetch` 存入原始 HTML；前端 `web-bridge.ts` 的 `FetchResult` 接口同步；`RemoteServicesPage.safeFetch` 改用 `result.raw`。注意 WebView 路径（桌面/移动端）暂不返回 `raw`（值为空串），仅 `http_fetch` 有效。
- **2026-07-24**: servicehub 目录中的服务从内置服务迁移时格式不规范——index.html 首行有 `<!-- AMIBA_BRIDGE -->`，packager 在 `<body>` 后又注入一个，造成双标记。容器 `replace` 只替换首个（DOCTYPE 前），bridge `<script>` 注入到 `<!DOCTYPE html>` 之前触发浏览器怪异模式。修复：移除 5 个 index.html 首行标记（widget HTML 保留，不走 packager）。建立新规范：index.html **不得**手动添加 `<!-- AMIBA_BRIDGE -->`，统一由 packager 在 `<body>` 后注入。|迁移原因：(1) 原实现用 `synchronized(lock).wait(60000)` 同步阻塞 tokio worker 线程，旋转屏幕 / Activity 重建时必闪退；(2) deprecated `startActivityForResult` 不感知生命周期，重建后回调丢失；(3) 手动 `unsafe` JVM 裸指针 + `Handler.post` 不可靠。新方案：AndroidX `ActivityResultRegistry` 异步回调 + Tauri PluginHandle 托管 JVM 生命周期，零阻塞。迁移改动：(a) `picker.rs` 删除 `pick_folder` command（-76 行），仅保留 `read_tombstone`；(b) `folder-picker.ts` 用动态 `import('tauri-plugin-android-fs-api')` 替代 `invoke('pick_folder')`；(c) `file-access-grants.ts` 从基于 POSIX 路径的授权模型改为基于 `content://` URI 的授权模型（+210 行改动）。注意：`MainActivity.kt` 中的 `FolderPickerHelper` 代码未删除（避免下次 `tauri android init` 重置后被遗漏），但已无调用路径。
