// ============================================================
// 变形虫 (Amiba) — 中文语言包 (zh-CN)
// ============================================================
import type { LocalesSchema } from '../types'

const zhCN: LocalesSchema = {
  app: {
    title: '变形虫',
    settings: '设置',
    memory: '记忆管理',
    service: '服务',
    services: '服务浏览',
    serviceBrowse: '服务浏览',
    newSession: '新会话',
    delete: '删除',
    quick: '快捷',
    registry: '服务仓库',
    updateAvailable: '发现新版本 v{version}',
    dismiss: '关闭',
  },
  quick: {
    emptyHint: '这是一个可自定义的快捷页面。告诉 AI 你想要什么，它会帮你定制这里的内容。',
    back: '返回',
  },
  apiSetup: {
    title: '配置 AI API',
    descMissing: '尚未配置 AI API。请填写以下信息，验证通过后即可开始使用。',
    descUnavailable: '当前 AI API 无法连接。请检查网络与以下配置，验证通过后即可继续使用。',
    testing: '正在测试连接…',
    testSuccess: '✓ 连接成功',
    testFailed: '✗ 连接失败：{error}',
    verify: '验证并继续',
  },
  chat: {
    newSession: '新会话',
    delete: '删除',
    messageCount: '条',
    noSessions: '暂无历史会话',
    sessions: '会话列表',
    emptyHint: '开始与 AI 对话吧',
    emptySubHint: '记忆会在对话中自动保存',
    thinking: '思考过程',
    thinkingProgress: '思考中...',
    placeholder: '输入消息...',
    send: '发送',
    sending: '...',
    stop: '停止',
    defaultSessionTitle: 'AI 对话',
    today: '今天',
    yesterday: '昨天',
    stats: {
      title: '统计',
      saveMemoryHint: '距离建议保存记忆',
      roundsLeft: '还有 {n} 轮',
      nearbyDevices: '附近设备',
      lanLabel: '局域网 (LAN)',
      bleLabel: '蓝牙 (BLE)',
      total: '总计',
      units: '台',
      noPeers: '暂无发现的设备',
      close: '关闭',
    },
    errorNoApiKey: '请先在设置中配置 API Key',
    errorPrefix: '错误',
    stepLimit: '已达 {n} 轮工具调用上限',
    stepLimitContinue: '继续处理',
    stepLimitEnd: '结束',
    stepLimitContinueMsg: '请继续处理未完成的任务。',
    reviewRunning: '正在整理技能库...',
    reviewComplete: '技能库整理完成 | 创建 {created} 个 / 修补 {patched} 个 / 删除 {deleted} 个',
    reviewSkipped: '审查跳过：消息不足或已有审查在进行中',
    reviewError: '审查失败：{error}',
    retry: '重新生成',
    retryHint: '此回复可能不完整',
  },
  settings: {
    title: '设置',
    tabs: { general: '通用', skills: '技能 & Agent', services: '服务', data: '数据', logs: '日志', appearance: '外观' },
    general: {
      apiConfig: 'API 配置',
      apiKey: 'API Key',
      baseUrl: 'Base URL',
      model: '对话模型',
      defaultProvider: '当前供应商',
      noProviderSelected: '-- 不使用供应商 --',
      customModelPlaceholder: '输入自定义模型',
      reasoningEffort: '思考努力程度',
      reasoningDefault: '-- 默认（不设置） --',
      reasoningLow: '低',
      reasoningMedium: '中（均衡）',
      reasoningHigh: '高（深入）',
      reasoningXhigh: '极高',
      reasoningMax: '最强',
      aiProvider: 'AI 供应商',
      providerNamePlaceholder: '显示名称',
      providerIdPlaceholder: '唯一 ID（英文）',
      providerBaseUrlPlaceholder: 'Base URL',
      providerApiKeyPlaceholder: 'API Key',
      providerModelsPlaceholder: '模型列表（每行一个）',
      defaultApiHint: '默认使用 Responses API，联网搜索由服务端执行并已开启',
      webSearch: '联网搜索',
      webSearchHint: '由供应商服务端执行搜索（如 DeepSeek v4）',
      save: '保存',
      cancel: '取消',
      noProviders: '暂无自定义供应商（将使用上方 API 配置作为默认）',
      addProvider: '添加供应商',
      modelCount: '个模型',
      appearance: '外观',
      themeMode: '主题模式',
      themeSystem: '跟随系统',
      themeLight: '浅色',
      themeDark: '深色',
      language: '语言',
      languageZh: '中文',
      languageEn: 'English',
      network: '网络',
      lanDiscovery: '局域网发现',
      lanDesc: '允许其他设备通过局域网发现本设备',
      lanVisible: '本设备在局域网中可见',
      lanHidden: '本设备在局域网中隐藏',
      about: '关于',
      aboutTitle: '变形虫 Amiba',
      aboutSubtitle: 'AI 驱动的跨平台即时应用平台',
      aboutStack: 'Vue 3 + TypeScript + Tauri',
      checkUpdate: '检查更新',
      checking: '检查中…',
      upToDate: '已是最新版本',
      newVersion: '发现新版本',
      directDownload: '直接下载',
      downloading: '正在下载…',
      cancelDownload: '取消下载',
      installing: '正在启动安装程序…',
      downloaded: '已下载',
      installNow: '立即安装',
      reDownload: '重新下载',
      downloadCancelled: '下载已取消',
      checkFailed: '检查更新失败',
      noPlatformAsset: '当前平台没有匹配的安装包',
      downloadFailed: '下载失败',
      installFailed: '安装启动失败',
      goToMarket: '鸿蒙版请前往应用市场更新',
      backgroundServices: '后台服务',
      backgroundDesc: '允许服务在后台持续运行（如定时提醒、设备监听等）',
      backgroundMax: '最大并发数',
      backgroundMaxHint: '控制同时运行的后台服务数量',
    },
    skills: {
      skillManagement: '技能管理',
      skillNamePlaceholder: '名称',
      skillDescPlaceholder: '描述',
      skillKwsPlaceholder: '关键词（逗号分隔）',
      skillTplPlaceholder: '模板（可选）',
      save: '保存',
      cancel: '取消',
      noSkills: '暂无自定义 Skill',
      importFolder: '导入 Skill 文件夹',
      importZip: '导入 ZIP',
      exportZip: '导出 ZIP',
      shareLan: '局域网分享 Skill',
      exporting: '正在导出...',
      edit: '编辑',
      importFromUrl: '从链接导入',
      importUrlPlaceholder: '输入 Skill ZIP 下载链接...',
      customAgent: '自定义 Agent',
      agentNamePlaceholder: '显示名称',
      agentIdPlaceholder: '唯一 ID（英文）',
      agentProviderDefault: '-- 选择供应商 --',
      agentModelDefault: '-- 选择模型 --',
      agentReasoningDefault: '-- 思考努力程度（默认） --',
      agentSystemPromptPlaceholder: '自定义 System Prompt（可选）',
      activate: '启用',
      noAgents: '暂无自定义 Agent（将使用默认 API 配置）',
      addAgent: '添加 Agent',
      newProviderName: '新供应商',
      newAgentName: '新 Agent',
    },
    data: {
      serviceImport: '服务导入',
      scanStorage: '扫描存储目录',
      selectFile: '选择文件',
      install: '安装',
      storage: '存储',
      exportConfig: '导出配置',
      deleteSessions: '删除聊天记录',
      deleting: '删除中…',
      deleteSessionsHint: '删除聊天记录不影响服务和记忆，清除所有数据不可恢复',
      clearAllData: '清除所有数据',
      reinstallPrebuilt: '重新安装内置服务',
      reinstallPrebuiltHint: '重新安装所有预置服务（如误删后需恢复）',
      reinstalling: '安装中…',
    },
    logs: {
      title: '日志管理',
      settings: '日志设置',
      enabled: '启用日志记录',
      level: '最低记录级别',
      levels: {
        debug: '调试',
        info: '信息',
        warn: '警告',
        error: '错误',
      },
      maxFiles: '最大文件数',
      maxSize: '单文件最大大小 (MB)',
      files: '日志文件',
      viewer: '日志内容',
      noLogs: '暂无日志文件',
      noContent: '请选择一个日志文件',
      search: '搜索日志…',
      filterLevel: '按级别过滤',
      columns: {
        time: '时间',
        level: '级别',
        module: '模块',
        message: '消息',
      },
      actions: {
        clearAll: '清空所有日志',
        deleteFile: '删除文件',
        export: '导出日志',
      },
    },
    appearance: {
      title: '界面外观',
      activeTheme: '当前主题',
      themeList: '主题列表',
      builtinTag: '内置',
      userTag: '自定义',
      createTheme: '保存当前为...',
      createPlaceholder: '新主题名称',
      deleteTheme: '删除',
      deleteConfirm: '确定删除主题 "{name}" 吗？',
      colorPreview: '配色预览',
      noUserThemes: '没有自定义主题',
      loading: '加载中...',
    },
    confirm: {
      clearAllData: '确定要清除所有数据吗？这将删除配置、记忆和已安装的服务。此操作不可撤销！',
      deleteSessions: '确定要删除所有聊天记录吗？此操作不可撤销！',
      importFailed: '导入失败',
      importZipFailed: 'ZIP 导入失败：{reason}',
      nameDescRequired: '名称和描述不能为空',
      providerFieldsRequired: '名称、ID 和 Base URL 不能为空',
      agentFieldsRequired: '名称、ID、供应商和模型不能为空',
      deleteProvider: '确定要删除供应商 "{name}" 吗？',
      deleteAgent: '确定要删除 Agent "{name}" 吗？',
      deleteSkill: '确定要删除 Skill "{name}" 吗？',
      installFailed: '安装失败',
      saved: '已保存',
      clearAllLogs: '确定要删除所有日志文件吗？此操作不可恢复。',
      deleteLogFile: '确定要删除此日志文件吗？',
    },
    dialect: {
      installedNServices: '已安装 {n} 个服务',
      noServicesFound: '未发现可安装的服务',
      deletedAllSessions: '已删除所有会话记录',
    },
    services: {
      remoteRepo: '远程服务仓库',
      registryUrl: '仓库地址',
      registryUrlPlaceholder: '输入远程服务仓库的 URL...',
      registryUrlHint: '仓库地址应为包含 index.json 的 HTTP(S) URL',
      browseRepo: '浏览远程仓库',
    },
  },
  memory: {
    title: '记忆管理',
    subtitle: 'AI 自动维护的持久记忆。条目用 § 分隔。',
    memoryPlaceholder: 'AI 的笔记（MEMORY.md）...',
    userPlaceholder: '用户画像（USER.md）...',
    save: '保存',
    revert: '还原',
    clear: '清空',
    entriesPreview: '条目预览 ({n} 条)',
    saved: '已保存',
    confirmClear: '确定要清空 {name} 吗？',
    tabRequirements: '需求',
    reqServiceCount: '{n} 个服务',
    reqEmpty: '暂无需求记录。需求由 AI 在服务迭代过程中自动追踪，也可在下方服务卡片中手动添加。',
    reqCurrent: '当前需求',
    reqOptimize: '待优化',
    reqFeedback: '用户反馈',
    reqDone: '已完成',
    reqMarkDone: '标记完成',
    reqAdd: '添加',
    reqAddPlaceholder: '添加新需求…',
    reqConfirmDelete: '确定删除这条需求吗？',
  },
  services: {
    title: '服务',
    importFolder: '导入',
    createNew: '新建',
    userServices: '用户服务',
    noUserServices: '还没有安装用户服务',
    ctaGenerate: '跟 AI 对话生成',
    noDescription: '暂无描述',
    systemServices: '系统服务',
    builtin: '内置',
    demoTitle: 'Hello World 示例',
    demoDesc: '快速安装一个演示服务体验功能',
    install: '安装',
    confirmDelete: '确定要删除 "{name}" 吗？此操作不可撤销。',
    dialogTitle: '选择服务目录',
    noManifestJson: '所选目录中没有 manifest.json',
    invalidManifest: 'manifest.json 格式无效：缺少 id 或 name',
    missingIndexHtml: '服务包必须包含 index.html',
    imported: '服务 "{name}" 已导入！',
    importFailed: '导入失败',
    widgetToggle: '显示悬浮块',
    serviceToggle: '启用服务',
    backgroundToggle: '后台运行',
    backgroundRunning: '后台运行中',
    backgroundStopped: '后台已停止',
    backgroundStart: '启动后台',
    registry: '服务仓库',
    ai: {
      title: 'AI 对话设置',
      enable: '启用 AI 对话',
      enableHint: '启用后服务内可通过 __amiba__.ai 发起 AI 对话；API Key 与工具执行均在宿主侧，服务无法直接接触。',
      grantConfirm: '服务「{name}」未声明 ai 权限，启用将为其补充权限声明。是否继续？',
      readonlyTools: '只读工具（默认开启）',
      sensitiveTools: '敏感工具（按需开启）',
      resetDefault: '恢复默认',
      tools: {
        doc_list: '列出可用文档',
        doc_read: '阅读文档内容',
        doc_search: '搜索文档',
        catalog_search: '搜索组件目录',
        service_list: '列出已安装服务',
        service_view: '查看服务详情',
        skills_list: '列出技能',
        skill_view: '查看技能内容',
        requirement_view: '查看服务需求',
        requirements_summary: '查看全局需求摘要',
        web_fetch: '获取网页内容',
        session_search: '搜索历史聊天（涉及隐私）',
        web_browse: '浏览器交互（会弹出窗口）',
        ui_theme_set_variables: '修改全局主题变量',
        ui_slot_set: '注入界面插槽内容',
      },
    },
    svcTools: {
      title: '服务工具',
      enable: '允许 AI 调用该服务的工具',
      enableHint: '服务经 __amiba__.tools.register 暴露的工具可被主聊天 AI 调用；仅在服务运行时可用。',
      grantConfirm: '服务「{name}」未声明 tools 权限，启用将为其补充权限声明。是否继续？',
      noTools: '该服务尚未提供工具（manifest.aiTools 声明或服务运行时注册后可见）。',
      sensitiveBadge: '敏感',
    },
  },
  host: {
    loading: '正在加载服务...',
    back: '返回',
    close: '关闭',
  },
  share: {
    title: '局域网分享',
    sendTab: '发送服务',
    receiveTab: '等待接收',
    selectService: '选择服务',
    selectPeer: '选择设备',
    send: '发送',
    waitForShare: '等待分享',
    waiting: '等待中...',
    noPeers: '未发现附近设备',
    noUserServices: '没有可分享的用户服务',
    progress: '传输中',
    complete: '完成',
    error: '错误',
    requestFrom: '{name} 想分享服务',
    confirm: '确认接收',
    decline: '拒绝',
    accepted: '已接受，接收中...',
    declined: '已拒绝',
    installing: '正在安装...',
  },
  skillShare: {
    title: '技能分享',
    sendTab: '发送技能',
    receiveTab: '等待接收',
    selectSkill: '选择技能',
    selectPeer: '选择设备',
    send: '发送',
    waitForShare: '等待分享',
    waiting: '等待中...',
    noPeers: '未发现附近设备',
    noUserSkills: '没有可分享的技能',
    progress: '传输中',
    complete: '完成',
    error: '错误',
    requestFrom: '{name} 想分享技能',
    confirm: '确认接收',
    decline: '拒绝',
    accepted: '已接受，接收中...',
    declined: '已拒绝',
    installing: '正在安装...',
    exportSuccess: '导出成功',
    importZipFailed: 'ZIP 导入失败',
    listenForSkill: '开始监听技能分享',
    skillFrom: '来自 {name}',
  },
  webview: {
    browsing: '正在浏览',
    closeTitle: '关闭 WebView',
    closeConfirm: '确定要关闭 WebView 吗？',
    confirm: '确定关闭',
    cancel: '取消',
    capturing: '截图中...',
  },
  registry: {
    title: '远程服务仓库',
    source: '仓库地址：',
    urlPlaceholder: '输入远程服务仓库的 URL...',
    go: '前往',
    loading: '正在获取服务列表...',
    retry: '重试',
    empty: '仓库中暂无可用服务',
    noUrl: '请先在设置中配置远程服务仓库地址',
    invalidIndex: '远程仓库 index.json 格式无效',
    fetchFailed: '获取服务列表失败',
    noDescription: '暂无描述',
    install: '安装',
    installing: '安装中...',
    update: '更新',
    installed: '已安装',
    updateAvailable: '可更新',
    installFailed: '安装失败',
    root: '根目录',
    folders: '文件夹',
  },
}

export default zhCN
