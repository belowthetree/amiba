...
const VALID_PERMISSIONS = ['storage', 'notification', 'widgets', 'network']
      ...
      parameters: {
        ...
        properties: {
          ...
          permissions: {
            type: 'array',
            description:
              '需要的权限列表。storage=数据持久化，notification=Toast通知，widgets=悬浮块，network=局域网P2P通信。多人/协作/聊天场景需要 network。',
            items: {
              type: 'string',
              enum: ['storage', 'notification', 'widgets', 'network'],
            },
          },
        },
